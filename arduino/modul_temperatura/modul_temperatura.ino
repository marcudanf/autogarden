...
void loop() {
  ...
  if ((m - last_post >= 30 minut ) || !last_post ) {
    ...
    HTTPClient http;
    String url = "http://192.168.43.31/upload/?";
...
up:
  ...
  /*if (request.indexOf("dt=") != -1) {
    dt = request.substring(request.indexOf("dt=") + 3, request.indexOf(" HTTP")).toFloat();
    EEPROM.begin(5);
    EEPROM.write(3, (byte)dt);
    EEPROM.commit();
    }*/
  readSensors(0);
  check();
  ...
  if (changed) {
    ...
    page += "<script>\n";
    /*page += "function new_dt(){\n";
      page += "var new_dt=prompt('Enter new \\u0394t:','";
      page += dt;
      page += "');\n";
      page += "if(new_dt!=null && new_dt!=''){\n";
    ...
    page += "C";
    page += "\n</button>\n<br/><br/>";
    /*page += "<br/><br/><button class='content' onclick='new_dt()'>&Delta;t:";
      page += dt;
      page += (char)176;
      page += "C";
      page += "\n</button>\n<br/><br/>\n";*/
